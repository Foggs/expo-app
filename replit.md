...
- **Security**: Comprehensive measures including Helmet for HTTP headers, rate limiting on API (200 req/15min) and WebSocket (1500 msg/min per connection), Zod validation for all inputs, body size limits, CORS, and WebSocket origin validation. Server uses lazy `clearStaleGameId()` at `join_queue`/`create_room`/`join_room` to clear stale completed/abandoned game refs only when a player proactively starts a new game — never during active gameplay. `cleanupConnection` clears the remaining player's `conn.gameId`/`conn.playerRole` immediately on opponent disconnect, allowing instant re-queuing.
